...
const EmailVerification = ({ email, signupId, onSuccess, onBack }) => {
  ...
  useEffect(() => {
    // Redirect if no email or signupId provided
    if (!email || !signupId) {
      onBack?.();
      return;
    }

    console.log('Email Verification - Email:', email, 'Signup ID:', signupId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://api.arvexpay.com/api/v1/auth/merchant/signup/verify-email', payload, {
        headers: {
      ...
      if (error.response?.data?.errors) {
        ...
        } else if (serverErrors.signup_id) {
          setError(serverErrors.signup_id[0]);
        } else {
  ...
  const handleResendCode = async () => {
    ...
    try {
      // For now, simulate API call - you may need to implement resend endpoint
